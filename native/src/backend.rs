...
const BACKEND_NAME: &str = "fleet-agent-mcp-backend.exe";
const BACKEND_PORT: u16 = 10996;
const BACKEND_TAG: &str = "fleet-agent-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "FLEET_AGENT_PORT";
const ENV_HOST: &str = "FLEET_AGENT_HOST";
const ENV_TAURI: &str = "FLEET_AGENT_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    // escalated kill (UAC), TIME_WAIT poll.
    #[cfg(windows)]
    {
        // Kill by image name (catches zombies NOT holding the port)
        let img_kill = format!(
            "Stop-Process -Name 'fleet-agent-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'fleet-agent-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM fleet-agent-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM fleet-agent-mcp-native.exe /T 2>$null"
        );
        ...
        // Poll up to 60s. Re-kill at 5s if first attempt failed.
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    log_line(
        &app,
        &format!("spawning {} (cwd {}) on port 10996",
