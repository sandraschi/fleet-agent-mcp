    ...
    return JSONResponse(
        {
            ...
            "subsystems": [
                ...
                {"name": "evolution", "count": 3, "annotation": "Correction log"},
                {
                    "name": "heartbeat",
                    "count": 3,
                    "annotation": "Wake-up + health + pipeline liveness",
                },
                {
                    "name": "fleet_bridge",
                    "count": 4,
                    "annotation": "Cross-server MCP client + list tools",
                },
                ...
                {"name": "notify", "count": 3, "annotation": "Email send + cron scheduler"},
                {
                    "name": "coworker",
                    "count": 3,
                    "annotation": "Portmanteau: execute flow + list + bootstrap",
                },
                ...
                {"name": "voice", "count": 1, "annotation": "Wake-word voice command routing"},
                {
                    "name": "scripts",
                    "count": 6,
                    "annotation": "Script CRUD + run for task scripting",
                },
            ],
        }
    )
...
async def api_health(request: Request) -> JSONResponse:
    ...
    try:
        tool_count = len(
            [v for v in _mcp.local_provider._components.values() if hasattr(v, "name")]
        )
    except Exception:
        pass
    ...
    try:
        ...
        if _db.exists():
            ...
            t = _conn.execute("SELECT count(*) FROM todo_items WHERE status='pending'").fetchone()[
                0
            ]
            task_pending = t
            _conn.close()
...
async def api_diagnostics(request: Request) -> JSONResponse:
    ...
    try:
        tool_names = sorted(
            v.name for v in _mcp.local_provider._components.values() if hasattr(v, "name")
        )
    except Exception:
        pass
...
def main() -> None:
    ...
    parser.add_argument(
        "--agentic", action="store_true", help="Enable CodeMode BM25 discovery transform"
    )
