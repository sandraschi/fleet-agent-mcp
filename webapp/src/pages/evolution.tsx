  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <button onClick={fetchEvolution} disabled={loading}
      ...
      {entries.length === 0 && !loading && (
        <p className="text-sm text-slate-500">No evolution entries yet. Fritz hasn't made any corrections.</p>
      )}

      <div className="grid grid-cols-1 gap-3">
        {entries.map((e) => (
          <div key={e.id} className="border border-slate-800 rounded-lg bg-slate-900/50 p-4">
            <div className="flex items-start gap-3">
              <span className="shrink-0 mt-0.5 w-2 h-2 rounded-full bg-amber-500" />
              <div className="min-w-0">
                <p className="text-sm font-medium text-amber-300">{e.correction}</p>
                <p className="text-sm text-slate-300 mt-1"><span className="text-fleet-400">Lesson:</span> {e.lesson}</p>
                {e.context && <p className="text-xs text-slate-500 mt-1">Context: {e.context}</p>}
                <p className="text-xs text-slate-600 mt-1">{new Date(e.created_at).toLocaleString()}</p>
              </div>
            </div>
          </div>
        ))}
