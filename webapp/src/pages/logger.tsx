...
import { Activity, Terminal, Trash2, RefreshCw, AlertTriangle, Info, XCircle } from "lucide-react";
    ...
    <div className="flex flex-col h-[calc(100vh-7rem)]">
      ...
      <div className="flex-1 overflow-y-auto space-y-0.5 font-mono text-xs">
        ...
        {filtered.map((entry) => (
          ...
          >
            <span className={`shrink-0 mt-0.5 ${LEVEL_COLORS[entry.level] ?? "text-slate-400"}`}>
              {levelIcon(entry.level)}
            </span>
            <span className="text-slate-600 shrink-0 w-20">
              {entry.timestamp ? entry.timestamp.slice(11, 23) : "--:--:--"}
            </span>
            <span className={`shrink-0 uppercase text-[10px] font-bold w-10 ${LEVEL_COLORS[entry.level] ?? ""}`}>
              {entry.level}
            </span>
            <span className="text-slate-500 shrink-0 w-16 text-[10px]">{entry.source}</span>
