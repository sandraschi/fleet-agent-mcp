...
import { Settings2, RefreshCw, Check, X, Eye, EyeOff, Save } from "lucide-react";
...
export function SettingsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.models?.length) {
          setTestResult({ ok: true, msg: `Connected — ${data.models.length} models found` });
          setModels(data.models);
        } else {
  ...
  const selectProvider = (id: string) => {
    ...
    setSettings({ ...settings, provider: id, base_url: prov?.defaultUrl ?? settings.base_url });
    setTestResult(null);
  };
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <section className="border border-slate-800 rounded-lg bg-slate-900/50 p-5">
        <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-4">LLM Provider</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            ...
            <div className="w-32">
              <label className="text-xs text-slate-500 uppercase tracking-wider">Timeout (s)</label>
              <input
                type="number"
                value={settings.timeout}
                min={5}
                max={300}
                onChange={(e) => setSettings({ ...settings, timeout: parseInt(e.target.value) || 60 })}
                className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:border-fleet-500 outline-none"
              />
            </div>
          </div>
          ...
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-wider">API Key (for OpenAI-compatible)</label>
      ...
      <div className="flex gap-3 items-center">
        ...
        {testResult && (
          <div className={`flex items-center gap-2 text-sm ${testResult.ok ? "text-emerald-400" : "text-red-400"}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {models.map((m) => (
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 ml-2">({(m.size / 1e9).toFixed(1)} GB)</span>
              </button>
            ))}
          </div>
        </section>
