    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="flex items-center gap-2 border border-red-800 rounded-lg bg-red-950/50 p-3 text-sm text-red-300">
          ...
          {error} — start the backend first (<code className="text-red-200 bg-red-900/50 px-1 rounded">.\start.ps1</code>)
        </div>
      )}
      ...
      {health && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <StatCard label="Active Workflow" value={health.active_workflow ?? "none"} />
          <StatCard label="Current Node" value={health.current_node ?? "—"} />
          <StatCard label="Pending Tasks" value={String(health.tasks.pending)} sub={`${health.tasks.done} done, ${health.tasks.total} total`} />
          <StatCard label="Memory Cards" value={String(health.memory_cards)} />
          <StatCard label="Evolution Entries" value={String(health.evolution_entries)} />
          <StatCard label="Registered Workflows" value={String(health.workflows_registered)} />
        </div>
      )}

      {!health && !loading && !error && (
        <p className="text-sm text-slate-500">Could not reach backend at /api/status. Start the server first.</p>
      )}
    </div>
  );
}
