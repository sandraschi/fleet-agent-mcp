...
const SUBSYSTEMS: Subsystem[] = [
	{
		...
		label: "FlowForge",
		count: 9,
		color: "text-fleet-400",
		tools: [
			{
				name: "workflow_define",
				type: "W",
				description:
					"Register a workflow from a YAML file. Enforces step-by-step execution with DAG of nodes.",
				params: [
					{
						name: "yaml_path",
						type: "str",
			...
			{
				name: "workflow_autodiscover",
				type: "W",
				description:
					"Auto-discover and register all YAML workflows from ./workflows/ and ~/.fleet-agent/workflows/.",
				params: [],
				returns:
			...
			},
			{
				...
				type: "W",
				description:
					"Start a new workflow instance from a registered workflow.",
				params: [
					{
						name: "name",
						type: "str",
						description: "Name of the registered workflow to start.",
						required: true,
					},
				],
				returns:
			...
			},
			{
				...
				params: [],
				returns:
					'{"success": bool, "workflow": str, "current_node": str, "task": str, "branches": list|null, "is_terminal": bool, "message": str}',
				examples: ["workflow_status()"],
			},
			{
				name: "workflow_next",
				type: "W",
				description: "Complete current node and advance to the next step.",
				params: [
					{
						name: "branch",
						type: "int | null",
						description:
							"Branch index to take (0-based). Required if current node has branches.",
						required: false,
						default: "null",
					},
				],
			...
			},
			{
				...
				params: [],
				returns:
					'{"success": bool, "workflow": str, "history": list[dict], "steps_completed": int, "message": str}',
		...
		tools: [
			{
				...
				type: "W",
				description:
					"Add a task to the unified TODO list. Tasks grouped by dependency: self, human, or external.",
				params: [
					{
						name: "task",
						type: "str",
					...
					},
					{
						name: "group",
						type: "str",
						description: "Dependency group: 'self', 'human', or 'external'.",
						required: false,
						default: '"self"',
					},
					{
					...
					},
					{
						name: "recurrence",
						type: "str | null",
						description: "Cron-style recurrence for repeating tasks.",
						required: false,
						default: "null",
					},
				],
			...
			},
			{
				name: "pulse_list",
				type: "R",
				description: "List all tasks, optionally filtered by group or status.",
				params: [
					{
						name: "group",
						type: "str | null",
						description: "Filter by group: 'self', 'human', 'external'.",
						required: false,
						default: "null",
					},
					{
			...
			},
			{
				name: "pulse_stale",
				type: "R",
				description: "Find tasks untouched for >= N days.",
				params: [
					{
						name: "days",
						type: "int",
						description: "Number of days without updates to flag as stale.",
						required: false,
						default: "3",
					},
				],
	...
	},
	{
		...
		tools: [
			{
				name: "memory_card_create",
				type: "W",
				description:
					"Create a knowledge card in the wiki. Each card captures a concept, pattern, lesson, or reference.",
				params: [
					{
						name: "title",
						type: "str",
					...
					},
					{
						...
						type: "str",
						description:
							"Category: general, pattern, project, mistake, reference.",
						required: false,
						default: '"general"',
					},
				],
				...
				examples: [
					'memory_card_create("SQLite WAL mode", "WAL provides concurrent reads...", tags=["sqlite", "performance"])',
				],
			},
			{
				name: "memory_card_search",
				type: "R",
				description: "Search knowledge cards by full-text query.",
				params: [
					{
						name: "query",
						type: "str",
						description: "Search query (matches title, content, and tags).",
						required: true,
					},
				],
				returns:
			...
			{
				name: "memory_card_update",
				type: "W",
				description:
					"Update a knowledge card's content and/or tags. Implements query-writeback.",
				params: [
					{
						name: "card_id",
						type: "str",
					...
					},
					{
						name: "tags",
						type: "list[str] | null",
						description: "New tags (replaces existing if provided).",
						required: false,
						default: "null",
					},
				],
			...
			{
				name: "memory_cards_list",
				type: "R",
				description: "List all knowledge cards.",
				params: [],
				returns:
					'{"success": bool, "cards": list[dict], "count": int, "message": str}',
			...
			},
			{
				...
				type: "R",
				description:
					"Lint the knowledge base for issues: broken references, stale cards, untagged cards. Prevents knowledge rot.",
				params: [],
				returns:
			...
			{
				name: "memory_project_note",
				type: "W",
				description:
					"Log a project observation or learning. Captures patterns, gotchas, architecture decisions.",
				params: [
					{
						name: "project",
						type: "str",
						description: "Project name (e.g. 'fleet-agent-mcp', 'flowforge').",
						required: true,
					},
					{
						name: "content",
						type: "str",
						description: "Note content — what you learned or observed.",
						required: true,
					},
					{
						name: "tags",
						type: "list[str] | null",
						description: "Tags for cross-referencing.",
						required: false,
						default: "null",
					},
				],
				...
				examples: [
					'memory_project_note("flowforge", "SQLite state survives session restarts", tags=["architecture", "persistence"])',
				],
			},
			{
				name: "memory_project_notes",
				type: "R",
				description: "List project notes, optionally filtered by project.",
				params: [
					{
						name: "project",
						type: "str | null",
						description: "Filter by project name. If omitted, lists all.",
						required: false,
						default: "null",
					},
				],
	...
	},
	{
		...
		tools: [
			...
			{
				name: "identity_north_star",
				type: "R",
				description:
					"Read the agent's NORTH_STAR.md — purpose, long-term goals, guiding principles. Used by pulse_align().",
				params: [],
				...
				params: [
					{
						name: "soul_path",
						type: "str",
						description: "Path to .soul file to inspect.",
						required: true,
					},
				],
				returns:
				...
				params: [
					{
						name: "soul_path",
						type: "str",
						description: "Path to .soul file to unpack.",
						required: true,
					},
					{
						name: "target_dir",
						type: "str | null",
						description:
							"Target directory for unpacking. Defaults to ~/.fleet-agent/.",
						required: false,
						default: "null",
					},
				],
		...
		tools: [
			{
				...
				type: "W",
				description:
					"Record a mistake, correction, and lesson in the evolution log. Every correction becomes a permanent lesson.",
				params: [
					{
						name: "correction",
						type: "str",
						description: "What went wrong and how it was fixed.",
						required: true,
					},
					{
						name: "lesson",
						type: "str",
						description:
							"The lesson learned — stated as a rule to follow going forward.",
						required: true,
					},
					{
						name: "context",
						type: "str",
						description: "What was being attempted when the mistake happened.",
						required: false,
						default: '""',
					},
				],
				...
				examples: [
					'evolution_record(correction="Used shell=True...", lesson="NEVER use shell=True for subprocess", context="State machine engine")',
				],
			},
			{
				name: "evolution_list",
	...
	},
	{
		...
		tools: [
			{
				name: "coworker_fleet_pulse",
				type: "W",
				description:
					"Morning Fleet Pulse — MCP health, git snapshot, hub publish.",
				params: [
					{
						name: "deliver",
						type: "bool",
						description: "Email report when SMTP configured.",
						required: false,
						default: "true",
					},
				],
				returns:
					'{"success": bool, "report": str, "artifact_path": str, "message": str}',
				examples: [
					"coworker_fleet_pulse()",
					"coworker_fleet_pulse(deliver=False)",
				],
			},
			{
				name: "coworker_devices_watch",
				type: "W",
				description:
					"Poll devices-mcp /api/fleet/priority — kitchen temp, CO, smoke, Ring.",
				params: [
					{
						name: "deliver",
						type: "bool",
						description: "Hub + urgent email on new critical.",
						required: false,
						default: "true",
					},
				],
			...
			{
				name: "coworker_day_prep",
				type: "W",
				description:
					"Office Day Prep — inbox + pulse tasks + AIWatcher hot items.",
				params: [
					{
						name: "deliver",
						type: "bool",
						description: "Email combined report.",
						required: false,
						default: "true",
					},
				],
			...
			{
				name: "coworker_bootstrap",
				type: "W",
				description: "Seed default coworker recurring tasks (idempotent).",
				params: [],
	...
	},
	{
		...
		tools: [
			{
				name: "intel_reports_publish",
				type: "W",
				description: "Publish markdown/HTML report to Intel Hub (port 11027).",
				params: [
					{
						name: "title",
						type: "str",
				...
				returns:
					'{"success": bool, "id": str, "url_path": str, "hub_url": str}',
				examples: ['intel_reports_publish(title="Test", markdown="# Hello")'],
			},
			{
				name: "intel_reports_list",
				type: "R",
				description: "List published intel reports from hub catalog.",
				params: [
					{
						name: "limit",
						type: "int",
				...
				returns:
					'{"success": bool, "reports": list, "hub_url": str, "count": int}',
				examples: ["intel_reports_list()", "intel_reports_list(limit=5)"],
			},
			{
				name: "aiwatcher_push_event",
				type: "W",
				description: "Push structured event into AIWatcher Fleet Events feed.",
				params: [
					{
						name: "title",
						type: "str",
	...
	return (
		<div className="space-y-8">
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-sm text-slate-400">
						{total} tools across {SUBSYSTEMS.length} subsystems
					</p>
				</div>
			...
			<div className="flex gap-2 text-xs text-slate-500">
				<span className="bg-blue-900/50 border border-blue-700 px-2 py-0.5 rounded">
					R = READ_ONLY
				</span>
				<span className="bg-amber-900/50 border border-amber-700 px-2 py-0.5 rounded">
					W = MUTATING
				</span>
				<span className="bg-red-900/50 border border-red-700 px-2 py-0.5 rounded">
					D = DESTRUCTIVE
				</span>
			</div>
			...
			{SUBSYSTEMS.map((ss) => (
				<div
					key={ss.id}
					className="border border-slate-800 rounded-lg bg-slate-900/50 p-5"
				>
			...
			{selected && (
				...
				>
					<div
						className="bg-slate-900 border border-slate-700 rounded-xl p-6 max-w-2xl w-full mx-4 shadow-2xl max-h-[85vh] overflow-y-auto"
						onClick={(e) => e.stopPropagation()}
					>
						<div className="flex items-center justify-between mb-4">
							<div className="flex items-center gap-3">
								<span
									className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border uppercase ${TYPE_BADGES[selected.type]}`}
								>
						...
						{selected.note && (
							<p className="text-xs text-amber-400 bg-amber-950/30 border border-amber-800/50 rounded p-2 mb-4">
								{selected.note}
							</p>
						)}
						<p className="text-xs text-slate-500 mb-4">
							<span
								className={`font-semibold ${selected.type === "R" ? "text-blue-400" : selected.type === "W" ? "text-amber-400" : "text-red-400"}`}
							>
								{TYPE_LABELS[selected.type]}
							</span>
							{" — "}
									...
									{selected.params.map((p) => (
										...
										>
											<div className="flex items-center gap-2 mb-1">
												<code className="text-sm font-mono text-fleet-300">
													{p.name}
												</code>
						...
						<div className="mb-4">
							...
							<pre className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs font-mono text-slate-300 overflow-x-auto whitespace-pre-wrap">
								{selected.returns}
							</pre>
						</div>
						...
						<div>
							...
							<div className="space-y-1">
								{selected.examples.map((ex, i) => (
									<pre
										key={i}
										className="bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs font-mono text-emerald-300 overflow-x-auto"
									>
										await {ex}
									</pre>
								))}
